...
form.addEventListener('submit', (event: Event) => {
  ...
  // maneira de resolver a questão do this numa Arrow Function (comparar com o arquivo A0019)
...
function checkForEmptyFields(...inputs: HTMLInputElement[]): void {
  inputs.forEach((input) => {
    // if (input.value === '') {
    // input.classList.add(SHOW_ERROR_MESSAGE);
    if (!input.value) {
...
function shouldSendForm(form: HTMLFormElement): boolean {
  ...
  //   if (item.classList.contains(SHOW_ERROR_MESSAGE)) {
  //     send = false;
  //   }
  // });
}
